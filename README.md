...
| `/fork` | 从当前对话复制一份到新会话（继承分叉点之前的完整历史），在新会话里自由探索替代方案。**分叉后的新会话是完全独立的——想返回主会话，按 Ctrl+D 退出分叉会话，再用 claude -c 恢复** |
...
| **隔离到新会话** | `/fork` | 想探索新方向但不确定结果，先分叉出去验证。`Ctrl+D` 退出返回 |
...
**如何返回主会话：** `/fork` 创建的是一个完全独立的 Claude Code 会话。验证完成后，按 `Ctrl+D`（或输入 `exit`）退出分叉会话，终端回到原始目录。再执行 `claude -c`（或 `/resume`）即可回到主会话继续工作。分叉会话和主会话互不影响——分叉里的改动不会出现在主会话里，除非手动迁移。

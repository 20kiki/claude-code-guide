...
- [斜杠命令](#-斜杠命令)
- [子 Agent 管理（/agents）](#agents--子-agent-管理)
- [上下文管理三件套](#-上下文管理三件套)
- [键盘快捷键](#-键盘快捷键)
- [对话中常用操作](#-对话中常用操作)
- [配置指南](#-配置指南)
...
| 快捷键 | 功能 | 使用场景 |
|--------|------|----------|
| `Ctrl + C` | 完全取消当前输出或操作，丢弃结果 | AI 完全跑偏时彻底中断，重新描述需求 |
| `Esc` | 中断当前输出，但保留已有部分 | AI 方向大致对但想调整，保留进度继续引导 |
| `Shift + Tab` | 循环切换权限模式，控制 AI 能自动执行哪些操作 | 关键文件用手动确认，样板代码用自动模式，详见下方说明 |
| `Esc Esc` | 双击清空当前输入 | 输入框内容乱了，快速清掉重写 |
...
| `\ + Enter` | 多行输入换行，不发送 |
| `Ctrl + T` | 切换任务列表面板 |
| `Ctrl + O` | 展开完整对话记录（transcript），可查看所有历史消息 |
| `Alt + T` | 切换扩展思考模式（extended thinking）开启/关闭 |
| `Alt + P` | 打开模型选择器 |
| `Ctrl + S` | 暂存当前输入内容并清空输入框。当你发完下一条消息后，暂存内容会自动恢复到输入框。适合写到一半临时插一个问题 |
| `Ctrl + G` | 在外部编辑器（$EDITOR）中打开当前输入 |
| `Ctrl + D` | 退出 Claude Code |
| `?`（英文问号） | 查看当前终端环境下所有可用快捷键。**注意必须是英文半角 `?`，中文 `？` 无效** |

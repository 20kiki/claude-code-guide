...
| `/compact` | 压缩对话上下文，保留关键信息、释放 token 空间。**上下文达 50% 时就主动执行**——超过 50% 后模型输出质量明显下降，不要等自动压缩（自动压缩在 ~95% 才触发，那时模型已处于最差状态） |
...
| `/fork` | 从当前对话复制一份到新会话（继承分叉点之前的完整历史），在新会话里自由探索替代方案。**分叉后的新会话是完全独立的——想返回主会话，在分叉会话中输入 /resume 选择主会话即可切回** |
...
| `/rename` | 重命名当前会话 |
| `/export` | 导出会话内容 |
| `/agents` | 管理子 Agent。查看、创建、配置专项子代理，详见下方说明 |
...
**执行 `/agents` 可以：**
- 查看当前所有可用的子 Agent
- 创建自定义子 Agent（存放在 `.claude/agents/` 目录）
- 为子 Agent 指定模型、工具权限、最大对话轮次
...
---
name: code-reviewer
description: 专项代码审查。写完代码后调用，检查安全和逻辑问题。
model: haiku
...
**为什么用子 Agent：** 探索代码库、跑测试、审查改动的过程会产生大量输出——这些留在子 Agent 的独立上下文中，返回给主对话的只有压缩后的结论，节省 token 并保持主对话清爽。
...
| **隔离到新会话** | `/fork` | 想探索新方向但不确定结果，先分叉出去验证。用 `/resume` 返回主会话 |
...
**如何返回主会话：** `/fork` 创建的是一个完全独立的 Claude Code 会话。验证完成后，在分叉会话中输入 `/resume`，从会话列表中选择主会话即可切回。分叉会话和主会话互不影响——分叉里的改动不会出现在主会话里，除非手动迁移。

**典型场景：**
- 重构到一半，想到另一种架构可能更简洁 → `/fork` 去验证，不行就 `Ctrl+D` 回来
...
**怎么做：** 输入 `/btw` 加上你的问题，回答会直接显示在终端中，完事即结束——不需要任何退出操作。主任务不受影响，可以继续推进。问题和回答只展示一次，绝不写入主对话历史。
...
| 快捷键 | 功能 |
|--------|------|
| `Ctrl + C` | 完全取消当前输出，丢弃结果。AI 完全跑偏时彻底中断重来 |
| `Esc` | 中断当前输出，保留已有部分。方向对但想微调时用 |
| `Shift + Tab` | 循环切换权限模式（default → acceptEdits → plan → auto），详见下方 |
| `Alt + T` | 切换扩展思考模式（extended thinking）开启/关闭 |
| `Alt + P` | 打开模型选择器 |
...
| `\ + Enter` | 多行输入换行，不发送 |
| `Esc Esc` | 双击清空当前输入框 |
| `Ctrl + S` | 暂存当前输入并清空。发完下一条消息后自动恢复。适合写到一半临时插一个问题 |
| `Ctrl + A` | 光标跳到行首 |
| `Ctrl + E` | 光标跳到行尾 |
| `Ctrl + U` | 删除光标前到行首 |
| `Ctrl + K` | 删除光标后到行尾 |
...
|--------|------|
| `Ctrl + T` | 切换任务列表面板 |
| `Ctrl + O` | 展开完整对话记录（transcript），查看所有历史消息 |
| `Ctrl + R` | 向上搜索命令历史 |
| `?`（英文问号） | 查看当前终端所有快捷键列表。**必须是英文半角 `?`，中文 `？` 无效** |

### Shift+Tab 权限模式详解

按 `Shift+Tab` 在以下四种模式间循环切换，决定 AI 在修改文件或执行命令时是否需要你逐项确认：

| 模式 | 自动批准范围 | 适用场景 |
|------|-------------|----------|
| **default**（默认） | 仅读取文件 | 刚开始使用、处理敏感代码、不确定 AI 会做什么时 |
| **acceptEdits**（接受编辑） | 读取 + 文件编辑 + 安全文件命令（`mkdir`、`mv`、`cp`、`touch`） | 信任 AI 改文件，但 Shell 命令仍需确认 |
| **plan**（计划模式） | 仅读取（不能写文件也不能执行命令） | 探索陌生代码库、复杂重构前先出方案 |
| **auto**（自动模式） | 全部操作（有后台安全分类器审核） | 长时间任务减少打断。需满足条件：Team/Enterprise 套餐 + Sonnet 4.6+ / Opus 4.6+ |

> 💡 **推荐策略：** 日常开发用 `acceptEdits`，改敏感代码切回 `default`，复杂重构先用 `plan` 出方案。`auto` 模式有安全分类器把关，默认阻止 `curl \| bash`、强制推送主分支、批量删除等危险操作。
...
在终端底部实时显示上下文占用百分比，方便在 50% 左右时及时 `/compact`。
...
5. **上下文 50% 就 `/compact`** — 超过一半后模型开始降智，等自动压缩（~95%）就晚了
